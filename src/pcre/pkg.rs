...
fn make_read_only(path: &Path) {
    do path.to_str().with_c_str |path_c_str| {
        let res = unsafe { libc::chmod(path_c_str, libc::consts::os::posix88::S_IRUSR as libc::mode_t) };
        if res != 0 {
            fail2!("Package script error: Failed to make `{}` read-only", path.to_str());
        }
    };
}
